...
Puppeteer-based end-to-end tests for the {{PROJECT_NAME_PASCAL}} UI.
...
# Specific test file
npx jest {{TESTS_DIR}}/browser/test_cache_debug.test.js
```
...
- `{{PROJECT_NAME_UPPER}}_URL=http://localhost:{{API_PORT}}` - Override base URL

## Requirements

- Node.js >= 18.0.0
- Puppeteer (auto-installs Chromium)
- {{PROJECT_NAME_PASCAL}} server running on port {{API_PORT}}
