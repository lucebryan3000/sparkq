 ...
 * Tests the happy path through the {{PROJECT_NAME_PASCAL}} UI
 */
...
describe('{{PROJECT_NAME_PASCAL}} Core UI Flow', () => {
  let browser;
  let page;
  ...
  test('should load the dashboard page with all tabs', async () => {
    ...
    expect(navbarBrand).toContain('{{PROJECT_NAME_PASCAL}}');
  ...
  test('should expose tab buttons for navigation', async () => {
    const projectTab = await page.$('.nav-tab[data-tab="{{PROJECT_NAME}}"]');
