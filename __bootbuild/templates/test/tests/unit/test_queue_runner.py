...
from {{PROJECT_NAME_SNAKE}} import queue_runner
from {{PROJECT_NAME_SNAKE}}.queue_runner import process_one
...
def mock_requests(monkeypatch):
    ...
    monkeypatch.setattr("{{PROJECT_NAME_SNAKE}}.queue_runner.requests.get", fake_get)
    monkeypatch.setattr("{{PROJECT_NAME_SNAKE}}.queue_runner.requests.post", fake_post)
    monkeypatch.setattr("{{PROJECT_NAME_SNAKE}}.queue_runner._http_request", fake_request)
    return history
...
def test_process_one_picks_oldest_and_completes(monkeypatch, mock_requests):
    ...
    monkeypatch.setattr("{{PROJECT_NAME_SNAKE}}.queue_runner.requests.get", fake_get)
    monkeypatch.setattr("{{PROJECT_NAME_SNAKE}}.queue_runner.requests.post", fake_post)
    monkeypatch.setattr("{{PROJECT_NAME_SNAKE}}.queue_runner._http_request", lambda method, url, **kwargs: fake_get(url, **kwargs) if method.upper() == "GET" else fake_post(url, **kwargs))
...
def test_acquire_lock_uses_env_dir_and_blocks_duplicate(tmp_path, monkeypatch):
    monkeypatch.setenv("{{PROJECT_NAME_SNAKE|upper}}_RUNNER_LOCK_DIR", str(tmp_path))
...
def test_acquire_lock_cleans_stale_lock(tmp_path, monkeypatch):
    ...
    stale_path = Path(tmp_path) / "{{PROJECT_NAME_SNAKE}}-runner-que_stale.lock"
...
def test_get_lock_dir_prefers_config(monkeypatch, tmp_path):
    monkeypatch.delenv("{{PROJECT_NAME_SNAKE|upper}}_RUNNER_LOCK_DIR", raising=False)
