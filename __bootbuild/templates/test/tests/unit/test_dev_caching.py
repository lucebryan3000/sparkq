...
def dev_client(tmp_path, monkeypatch):
    ...
    from {{SRC_DIR}} import api
    from {{SRC_DIR}} import env as env_module

    env_module.reset_env_cache()
    monkeypatch.setenv("{{PROJECT_NAME_SNAKE|upper}}_ENV", "dev")
...
def test_cache_buster_endpoint_exposes_env_and_cache_buster(dev_client):
    ...
    body = response.text
    assert 'window.__{{PROJECT_NAME_SNAKE|upper}}_ENV__ = "dev";' in body
    assert "window.__{{PROJECT_NAME_SNAKE|upper}}_CACHE_BUSTER__" in body
    assert "window.__{{PROJECT_NAME_SNAKE|upper}}_BUILD_ID__" in body
