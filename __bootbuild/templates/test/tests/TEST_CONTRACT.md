# {{PROJECT_NAME_PASCAL}} Test Contract

This contract defines the invariant {{PROJECT_NAME_PASCAL}} test coverage rules. Any new public surface must satisfy the matrix below; CI may block merges via `tools/test_index.py --fail-on-missing`.

## Coverage Matrix
- **API**: Every FastAPI route in `{{SRC_DIR}}/api.py` has:
  - Unit/integration coverage in `{{TESTS_DIR}}/integration/test_api_validation.py` (minimal validation for new routes is acceptable).
  - Additional route-specific files may exist but do not replace the baseline entry above.
- **CLI**: Every Typer command in `{{SRC_DIR}}/cli.py` (including sub-commands) is exercised in `{{TESTS_DIR}}/integration/test_cli.py`.
- **Storage**: Every public `Storage` method (non-underscore) in `{{SRC_DIR}}/storage.py` has coverage in `{{TESTS_DIR}}/unit/test_storage.py` (happy path + basic error/edge expectations per method type).
- **UI pages**: Every page registered under `ui/pages/*.js` (`window.Pages.<Name>` or filename) is covered by a browser test:
  - Primary: `{{TESTS_DIR}}/browser/test_<page>_page.test.js` or an entry in the shared page matrix-driven smoke test.
...
- The reusable prompt `_build/prompts/{{PROJECT_NAME}}-sync-tests.md` consumes the indexer output and applies the test patterns to fill gaps.

## Legacy Artifacts (cleanup candidates)
- Top-level CJS one-offs in `{{TESTS_DIR}}/` (`test-console-logs.cjs`, `test-debug.cjs`, `test-network.cjs`, `test-queue-ops.cjs`, `test-ui-queue-operations.cjs`, `test-ui-simple.cjs`) appear unused by current Jest config; remove or fold into matrix-driven tests once coverage parity is confirmed.
- `{{TESTS_DIR}}/logs/` should remain git-ignored or cleaned regularly; it is not a source of truth.
