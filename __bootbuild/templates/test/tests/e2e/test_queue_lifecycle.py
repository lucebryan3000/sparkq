...
Tests the complete lifecycle of tasks through {{PROJECT_NAME_PASCAL}} without browser
"""
import json
import os
import re
...
from {{SRC_DIR}}.cli import app
from {{SRC_DIR}}.storage import Storage
from {{SRC_DIR}}.tools import reload_registry
from {{SRC_DIR}} import paths
...
def queue_runner(tmp_path):
    ...
    with runner.isolated_filesystem(temp_dir=tmp_path) as temp_dir:
        ...
        config = {
            ...
            "database": {"path": "{{PROJECT_NAME_SNAKE}}.db", "mode": "wal"},
        ...
        Path("{{PROJECT_NAME_SNAKE}}.yml").write_text(yaml.safe_dump(config, sort_keys=False))
        Path("scripts").mkdir(exist_ok=True)

        # Initialize storage
        storage = Storage("{{PROJECT_NAME_SNAKE}}.db")
        storage.init_db()
        storage.create_project(name="queue-test", repo_path=str(temp_dir), prd_path=None)

        reload_registry()
        paths.reset_paths_cache()
        os.environ["{{PROJECT_NAME_UPPER}}_CONFIG"] = str(Path.cwd() / "{{PROJECT_NAME_SNAKE}}.yml")
...
class TestQueueLifecycle:
    ...
    def test_task_metadata_preservation(self, queue_runner):
        ...
        metadata = {
            ...
            "context": {"repo": "{{PROJECT_NAME_SNAKE}}", "branch": "main"},
        }
        ...
        assert payload["metadata"]["context"]["repo"] == "{{PROJECT_NAME}}"
