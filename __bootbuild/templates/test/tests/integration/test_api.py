...
def storage(tmp_path, monkeypatch):
    db_path = tmp_path / "{{PROJECT_NAME_SNAKE}}_test.db"
    ...
    from {{SRC_DIR}} import tools
    tools.reload_registry(config={"tools": {}, "task_classes": {k: {"timeout": v} for k, v in tools.TASK_CLASS_TIMEOUTS.items()}})
    # Point config path to test-specific {{PROJECT_NAME_SNAKE}}.yml if present
    paths.reset_paths_cache()
    os.environ["{{PROJECT_NAME_UPPER}}_CONFIG"] = str(tmp_path / "{{PROJECT_NAME_SNAKE}}.yml")
    return storage
...
def script_index_env(tmp_path, monkeypatch):
    ...
    (tmp_path / "{{PROJECT_NAME_SNAKE}}.yml").write_text(
        "script_dirs:\n"
        "  - scripts\n",
        encoding="utf-8",
    )
    monkeypatch.setattr(api, "CONFIG_PATH", tmp_path / "{{PROJECT_NAME_SNAKE}}.yml")
...
class TestTaskEndpoints:
    ...
    def test_create_task(self, api_client, storage_with_stream):
        ...
        from {{SRC_DIR}}.models import TaskClassDefaults
