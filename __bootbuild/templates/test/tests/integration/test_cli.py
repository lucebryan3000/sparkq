...
from {{SRC_DIR}}.cli import app, get_storage
...
tools:
  ...
  run-python:
    ...
    task_class: MEDIUM_SCRIPT
"""
    Path("{{PROJECT_NAME_SNAKE}}.yml").write_text(config)
...
def cli_runner():
    ...
    with runner.isolated_filesystem():
        ...
        get_storage().db_path = "{{PROJECT_NAME_SNAKE}}.db"
        get_storage().init_db()
...
class TestTaskCommands:
    ...
    def test_enqueue_invalid_tool(self, cli_runner: CliRunner):
        ...
        assert "{{PROJECT_NAME_SNAKE}} list tools" in result.stderr
    ...
    def test_claim_without_stream_errors(self, cli_runner: CliRunner):
        ...
        assert "{{PROJECT_NAME_SNAKE}} queue list" in result.stderr
