# {{PROJECT_NAME_PASCAL}} Test Patterns (v1)
...
- Location: `{{TESTS_DIR}}/integration/test_api_validation.py`.
- Pattern:
...
- Location: `{{TESTS_DIR}}/integration/test_cli.py`.
- Pattern:
...
## Storage Unit (pytest)
- Location: `{{TESTS_DIR}}/unit/test_storage.py`.
- Pattern:
...
- Location: `{{TESTS_DIR}}/browser/test_<page>_page.test.js` or matrix-driven smoke test.
- Pattern:
...
## Prompt Usage
- The sync prompt `_build/prompts/{{PROJECT_NAME}}-sync-tests.md` should direct Codex/Claude to:
