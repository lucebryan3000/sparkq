...
ROOT_DIR = Path(__file__).resolve().parent.parent  # {{PROJECT_NAME_SNAKE}}/
...
# Conditional storage import - skip tests if module not available
try:
...
def pytest_sessionfinish(session, exitstatus):
    ...
    with open(summary_file, "w") as f:
        ...
        f.write(f"{'='*80}\n")
        f.write(f"Timestamp: {datetime.now(ZoneInfo('{{TZ}}')).strftime('%m-%d-%Y %I:%M:%S %p CST')}\n")
